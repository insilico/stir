...
# ReliefF with $k=\lfloor(m-1)/6\rfloor$ is similar to multiSURF:
...
t4 <- ggplot(pval.melt, aes(x = attribute, y = value, group = variable, color = variable)) + 
  ...
  labs(y = "-Log10(p-value)", x = "Features (in data order)", title = "Significance of attributes (-log10(p))") + 
  theme_bw() +
...
# Plot of -log10(p-values) of attributes. p-values $< e^{-10}$ are plotted as $< e^{-10}$ for scaling. 
# Feautres are in their original order from the data, but the significant attributes tend to be on the left
...
# However, for interactions, some attributes on the right may be functional
# due to network co-exression. 
